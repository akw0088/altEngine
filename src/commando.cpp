...
// Need to delete this file, so old it's useless
...
void Commando::init(Engine *altEngine)
{
	engine = altEngine;
	create_crosshair();
...
void Commando::unload()
{
	last_spawn = 0;
}

void Commando::destroy()
{

}

void Commando::init_camera(vector<Entity *> &entity_list)
{
...
void Commando::handle_player(int self)
{
...
void Commando::handle_plasma(Player &player, int self)
{
	Frame camera_frame;
...
void Commando::handle_rocketlauncher(Player &player, int self)
{
	Frame camera_frame;
...
void Commando::handle_grenade(Player &player, int self)
{
	Frame camera_frame;
...
void Commando::handle_machinegun(Player &player, int self)
{
	char cmd[80] = { 0 };
	int index[8];
	int num_index;
...
void Commando::handle_weapons(Player &player, input_t &input, int self)
{
	bool fired = false;
	bool empty = false;
...
void Commando::draw_flash(Player &player)
{
...
void Commando::render_hud(double last_frametime)
{
...
void Commando::draw_name(Entity *entity, Menu &menu, matrix4 &real_projection)
{
	matrix4 trans2;
	matrix4 mvp2;
	matrix4 model;
...
void Commando::draw_line(Entity *ent_a, Entity *ent_b, Menu &menu, vec3 &color)
{
	matrix4 trans2;
	matrix4 mvp2;
	matrix4 model;
...
void Commando::transform_3d_2d(vec3 &position, vec3 &pos2d, matrix4 &projection)
{
	matrix4 matrix;
	matrix4 trans2;
	matrix4 mvp2;
...
int Commando::bot_get_path(int item, int self, int *nav_array, path_t &path)
{
...
int Commando::bot_follow(path_t &path, int *nav_array, Entity *entity, float speed_scale)
{
	static int timer = 0;
...
void Commando::console(int self, char *cmd, Menu &menu, vector<Entity *> &entity_list)
{
...
void Commando::setup_func(vector<Entity *> &entity_list, Bsp &q3map)
{
	for (unsigned int i = 0; i < entity_list.size(); i++)
	{
