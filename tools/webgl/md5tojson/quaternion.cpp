...
#include <cmath> // for acos, sinetc, slerp must be smooth
...
void quaternion::to_quat(matrix3 &mat)
{
	...
	if (trace > 0.0f)
	{
		s = sqrt(1.0f + mat.m[0] + mat.m[4] + mat.m[8]) / 2.0f;
		float s4 = (4.0f * s);
	...
	{
		float root = sqrt(1.0f + mat.m[0] - mat.m[4] - mat.m[8]) * 2; // S=4*qx 
	...
	{
		float root = sqrt(1.0f + mat.m[4] - mat.m[0] - mat.m[8]) * 2; // S=4*qy
	...
	{
		float root = sqrt(1.0f + mat.m[8] - mat.m[0] - mat.m[4]) * 2; // S=4*qz
...
void quaternion::slerp(const quaternion &p, const quaternion &q, float time, quaternion &result)
{
	...
	*/
	...
	result = (p * ((float)sin(omega * (1.0f - time))) + (q * (float)sin(omega*time))) / (float)sin(omega);
}
