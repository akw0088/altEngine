...
void MD5::PrepareMesh(int mesh_index, md5_joint_t *skeleton, int &num_index, int *index_array, vertex_t *vertex_array, int &num_vertex)
{
	...
	for (i = 0; i < mesh->num_vertex; ++i)
	{
		...
		{
			md5_weight_t *weight = &mesh->weight[mesh->vertex[i].start + j];
...
int MD5::load_md5(char *file)
{
	...
	joint = new md5_joint_t[num_joint + 1];
	if (joint == NULL)
	{
	...
	mesh = new md5_mesh_t[num_mesh + 1];
	if (mesh == NULL)
	{
		perror("malloc() failed");
		delete[] joint;
...
int MD5::parse_joint(char *data, md5_joint_t *joint, int num_joint)
{
	...
	printf("var animation = [];\n");

	printf("// Base Skeleton\n");
	printf("const base = '{\"frame\":[ \\\n");
	pdata = data;
	...
	while (pdata)
	{
		...
		if (8 == sscanf(pdata, "%255s %d ( %f %f %f ) ( %f %f %f )", name, &parent, &x, &y, &z, &qx, &qy, &qz))
		{
			...
			matrix3 mat;
			mat = joint[i].orient.to_matrix();


			if (i == num_joint - 1)
			{
				printf("\t { \"x\":\"0x%08X\", \"y\":\"0x%08X\", \"z\":\"0x%08X\", \"matrix\":[ \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\" ], \"name\":%s, \"parent\":\"%d\" } \\\n",
					*((unsigned int *)&x), *((unsigned int *)&y), *((unsigned int *)&z),
					*((unsigned int *)&mat.m[0]), *((unsigned int *)&mat.m[1]), *((unsigned int *)&mat.m[2]),
					*((unsigned int *)&mat.m[3]), *((unsigned int *)&mat.m[4]), *((unsigned int *)&mat.m[5]),
					*((unsigned int *)&mat.m[6]), *((unsigned int *)&mat.m[7]), *((unsigned int *)&mat.m[8]),
					name, parent
				);
			}
			else
	...
	printf("]}'; \n");
	printf("animation.push(JSON.parse(base));\n");
...
void MD5::build_frame(md5_joint_t *joint, float *frame, md5_anim_t *anim)
{
	...
	static int count = 0;
	printf(" // Frame %d Skeleton\n", count);

	printf("const frame%d = '{\"frame\":[ \\\n", count);
	...
	for (i = 0; i < anim->num_joint; i++)
	{
		...
		matrix3 mat_orig;
		mat_orig = joint[i].orient.to_matrix();

		matrix3 mat_parent;
		mat_parent = joint[joint[i].parent].orient.to_matrix();
		...
		if (joint[i].parent >= 0)
		{
			mat_copy = mat_parent * mat_orig;
		}

		if (i == anim->num_joint - 1)
		{
			printf("\t { \"x\":\"0x%08X\", \"y\":\"0x%08X\", \"z\":\"0x%08X\", \"matrix\":[ \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\", \"0x%08X\" ], \"name\":%s, \"parent\":\"%d\" } \\\n",
				*((unsigned int *)&joint[i].pos.x), *((unsigned int *)&joint[i].pos.y), *((unsigned int *)&joint[i].pos.z),
				*((unsigned int *)&mat.m[0]), *((unsigned int *)&mat.m[1]), *((unsigned int *)&mat.m[2]),
				*((unsigned int *)&mat.m[3]), *((unsigned int *)&mat.m[4]), *((unsigned int *)&mat.m[5]),
				*((unsigned int *)&mat.m[6]), *((unsigned int *)&mat.m[7]), *((unsigned int *)&mat.m[8]),
				joint[i].name, joint[i].parent
			);
		}
		else
		{
	...
	printf("animation.push(JSON.parse(frame%d));\n", count);
	count++;
