...
int gltf_load_external_bin(char * gltfFilename)
{
...
//	std::string binFilename = root["buffers"][0]["uri"].asString();
	...
	for (int i = 0; i < num_mesh; i++)
	{
		...
		// Now get the start of the float3 array by adding the bufferView byte offset to the bin pointer
		// It's a little sketchy to cast to a raw float array, but hey, it works.
		float* position_buffer = (float*)(binfile + position_BufferView["byteOffset"].asInt());
		float* normal_buffer = (float*)(binfile + normal_BufferView["byteOffset"].asInt());
		float* tangent_buffer = (float*)(binfile + tangent_BufferView["byteOffset"].asInt());
		float* texture_buffer = (float*)(binfile + texture_BufferView["byteOffset"].asInt());

		unsigned int* index_buffer = (unsigned int*)(binfile + index_BufferView["byteOffset"].asInt());
		unsigned short* index_buffer_short = (unsigned short*)(binfile + index_BufferView["byteOffset"].asInt());
